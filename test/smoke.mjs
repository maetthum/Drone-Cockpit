 ...
 *
 * Prüft das erzeugte Bild direkt (`shadow.lastImageUrl`), nicht den
 * gezeichneten Kartenausschnitt: MapLibres `image`-Source lädt das Bild
 * intern asynchron (auch bei einer `data:`-URL ein echtes `Image`-Decode) —
 * ohne zusätzliche, unklare Wartezeit ist nicht sicher zu sagen, wann das
 * tatsächlich auf dem Bildschirm steht. Das Bild selbst liegt dagegen sofort
 * nach `waitForIdle()` fest.
 */
