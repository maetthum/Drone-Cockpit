...
/**
 * Reine Objektkennungen taugen nicht als Überschrift.
 *
 * Manche Dienste — die elektrischen Anlagen etwa — führen als Bezeichnung nur
 * die UUID des Datensatzes. Die stand dann fett über dem Treffer, während der
 * sprechende Name („L0208 – Freienbach – Wädenswil") erst in den Sachdaten
 * darunter auftauchte.
 */
    ...
    async function query(lngLat) {
        ...
        try {
            ...
            treffer.forEach((hit, index) => {
                ...
                let titel = titleOf(properties);
                // Statt der Kennnummer die erste sprechende Zeile der
                // Sachdaten. Sie fällt unten aus der Liste, weil der Filter
                // Zeilen entfernt, die den Titel wiederholen.
                if (UUID.test(titel)) {
                    titel = rows.find((row) => row.value && !UUID.test(row.value))?.value ?? null;
                }
                ...
                if (titel !== null) children.push(line(titel, 'info-title'));
