...
export function createOverlays(map) {
    ...
    OVERLAYS.forEach((overlay) => {
        map.addSource(sourceId(overlay), {
            ...
            /*
             * **Nur hier nötig (`ortsnamen`), sonst überall 0.** Layer-
             * `minzoom` (unten) prüft `map.getZoom()` — einen einzigen
             * Skalarwert für die ganze Kamera. Bei der steilen Cockpit-
             * Neigung (80°+) bleibt der praktisch immer über 11, weil er den
             * Nahbereich misst; nachgemessen fordert MapLibre für den
             * Horizont dabei trotzdem Kacheln der Zoomstufen 7–13 an — die
             * Layer-Grenze griff dort also nie. Source-`minzoom` wirkt
             * dagegen je angeforderter Kachel: fehlt eine Stufe darunter,
             * holt MapLibre stattdessen mehrere Kacheln der Minimalstufe statt
             * einer groben — genau das lesbare Bild statt des Gewimmels.
             */
        ...
        map.addLayer({
            ...
            // Zusätzlich zum Source-`minzoom` oben: deckt den flachen Fall ab
            // (Blick von oben, `map.getZoom()` selbst unter der Grenze) — bei
            // Neigung greift dort vor allem die Source-Grenze. Nur
            // `ortsnamen` setzt das — die Ortssymbole überlagern sich bei
            // ganz CH im Bild zu einer ununterscheidbaren
            // Fläche, siehe config.js.
            minzoom: overlay.minzoom ?? 0
        });
    });
